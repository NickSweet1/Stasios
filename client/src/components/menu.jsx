  ...
  return (
    ...
    >
      ...
      <div className="text-3xl md:text-5xl lg:text-6xl text-center mb-6 relative">
        ...
        <br />
        <span className=" text-[30px] text-amber-800 opacity-50 sm: flex flex-wrap justify-center">
          <span
            className={`border-b-4 cursor-pointer ${
              showSandwichMenu ? "border-sgreen" : ""
            }`}
          ...
          <span
            className={`border-b-4 cursor-pointer ${
              showCoffeeMenu ? "border-sred" : ""
            }`}
            onClick={showCoffeeMenuHandler}
          >
            <span className="hidden md:inline-block">Coffees</span>
            <span className="md:hidden">Cafe</span>{" "}
            {/* Shortened text for mobile */}
          </span>
        </span>{" "}
        {/* Smaller and light grey */}
      </div>
      {showSandwichMenu && <SandwichMenu />}
      {showDessertMenu && <DessertMenu />}
